test_that("sample_size_fig returns a ggplot2 object", {
  ...
  expect_error(
    sample_size_fig(data = D_mb, species.name = "A. longiremis", ecodistrict.select = "Scotian Shelf"),
    "Species not found in data"
  )
  expect_error(
    sample_size_fig(data = D_mb, species.name = "Acartia longiremis", ecodistrict.select = "Bay of Fundy"),
