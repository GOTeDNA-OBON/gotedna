...
#' )
#' }
field_sample_fig <- function(newprob, year, global_max_size, max_px = 1) {
  ...
  plot_title <- if (n_species == 1) species_levels else NULL
  ...
  plotly::plot_ly(
    ...
    colors = colors,
    size = ~size_fraction,  # fractions 0-1
    marker = list(
      sizemode = "diameter",
      sizeref = 1 / max_px,  # fraction=1 → max_px
      sizemin = 4
    ),
    text = ~paste(
