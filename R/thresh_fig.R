...
#' @param threshold (required, character): Detection probability threshold for which
#' data are to be displayed to visualize potential optimal detection windows.
...
thresh_fig <- function(taxon.level, taxon.name, threshold, ecodistrict.select) {
  ...
  thresh = data.frame(values=c("thresh50","thresh55","thresh60","thresh65","thresh70", "thresh75", "thresh80", "thresh85", "thresh90", "thresh95"),
  ...
  print(ggplot2::ggplot() +
          ggplot2::geom_hline(mapping=ggplot2::aes(yintercept=y),
                              data.frame(y=c(0:4)/4),
                              color="darkgrey") +
          ggplot2::geom_vline(mapping=ggplot2::aes(xintercept=x),
                              data.frame(x=1:12),
                              color="lightgrey") +
          ggplot2::geom_col(
          ...
          ggplot2::labs(x=NULL, y="Normalized detection probability",
