...
hm_fig <- function(taxon.level, taxon.name, ecodistrict.select) {
  ...
  print(ggplot2::ggplot(
    ...
    ggplot2::aes(x=month, y=reorder(species, dplyr::desc(species))))+
      ggplot2::geom_rect(xmin=1,xmax=1.1,ymin=1,ymax=1.1,
                         linewidth=0,ggplot2::aes(colour="No data"))+ # this is to create the "No data" legend entry, but if there's another way then perfect
